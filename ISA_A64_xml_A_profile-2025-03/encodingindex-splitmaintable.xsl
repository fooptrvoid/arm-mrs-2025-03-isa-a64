 ...
 method="html" encoding="utf-8"/>
  ...
  <xsl:template match="hierarchy">
    <h1 class="topleveltable">
      ...
      <xsl:text> instruction set encoding</xsl:text>
    </h1>
  ...
  <xsl:template match="node">
    ...
    <xsl:if test="@groupname">
      ...
      <div class="decode_navigation">
        <xsl:choose>
          <xsl:when test="../@groupname">
            <p>The encodings in this section are decoded from <a href="#{../@groupname}"><xsl:value-of select="../header"/></a>.</p>
          </xsl:when>
          <xsl:otherwise>
            <p>The encodings in this section are decoded from <a href="#top"><xsl:value-of select="/encodingindex/@instructionset"/> instruction set encoding</a>.</p>
          </xsl:otherwise>
        </xsl:choose>
      </div>
  ...
  <xsl:template match="encodingindex/iclass_sect">
    ...
    <div class="iclass" id="iclass-{@id}">
      ...
      <xsl:if test="$group">
        <p>The encodings in this section are decoded from <a href="#{$group}"><xsl:value-of select="$groupheader"/></a>.</p>
      </xsl:if>
      <xsl:apply-templates/>
