...
-->
  ...
  <xsl:template match="desc">
    <xsl:choose>
      <xsl:when test="../docvars/docvar[@key = 'isa' and @value = 'A64'] or (not(./authored) and not(./longer))">
        <xsl:apply-templates select="brief"/>
      </xsl:when>
      <xsl:when test="./brief and ./longer">
        <p><xsl:value-of select="./brief"/><xsl:value-of select="./longer"/></p>
      </xsl:when>
    </xsl:choose>
  ...
  <xsl:template match="desc/brief">
    <xsl:choose>
      ...
      <xsl:otherwise> <!-- this case should be deprecated once the structure of brief is standardized -->
...
<!--
 ...
 -->
  ...
  <xsl:template match="/instructionsection/classes/iclass">
    ...
    <!-- Decode pseudocode -->
    <xsl:if test="ps_section">
      <xsl:choose>
        <xsl:when test="count(encoding) = 1">
          <h4>Decode for this encoding</h4>
        </xsl:when>
        <xsl:otherwise>
          <h4>Decode for all variants of this encoding</h4>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:if>
        ...
        <h4 class="encoding">
          <xsl:choose>
            <xsl:when test="@label and @label != ''">
              Encoding for the <xsl:value-of select="@label"/> variant
            </xsl:when>
            <xsl:otherwise>
 ...
 <xsl:template match="definition/intro">
    <!-- Wrap in paragraph and add "encoded in <blah>" if no children (expecting only text) -->
    <xsl:choose>
      <xsl:when test="count(*) - count(a) = 0">
        <p>
          <xsl:apply-templates/>
          encoded in
          <xsl:choose>
  ...
  <xsl:template match="operationalnotes/operationalnote">
    <xsl:apply-templates/>
    <xsl:if test="count(./operationalnote) > 1"><p/></xsl:if>
  </xsl:template>

  <xsl:template match="operationalnote/operationalnote_titles">
    <p class="aml">
      For the
      <xsl:for-each select="./operationalnote_title">
        <xsl:if test="position() != 1">
          <xsl:text>, </xsl:text> <!-- Comma before every entry other than the first. -->
        </xsl:if>
        "<xsl:apply-templates/>"
      </xsl:for-each>
      variant<xsl:if test="count(./operationalnote_title) > 1">s</xsl:if>:
    </p>
  </xsl:template>

  <xsl:template match="operationalnote/operationalnote_content">
    <xsl:apply-templates/>
  </xsl:template>
  ...
  <xsl:template match="xref">
    <a class="armarm-xref" title="Reference to Armv8 ARM section" data-linkend="{@linkend}"><xsl:value-of select="."/></a>
  </xsl:template>
